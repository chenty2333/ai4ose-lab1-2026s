...
这几天 ai4ose 都没有提交, 是去做我自己的操作系统了. 对各个模块都做了重构.

### 事件2: 完成组件化 ch4

完成了组件化 rCore 的 ch4, 比原版 rCore 简单很多. 原版需要在 memset 和 memarea 中提供新的 api, 需要手动管理 pagetable.

现在抽象程度高太多了, 拿到 start_vpn 和 end_vpn 之后验证权限位, 直接 process.address_space.map 就结束战斗了.

对于已经学过一遍 rCore 来说非常的简单. 但感觉对于不了解原始版本 rCore 的同学来说, 这样是不是也太 ‘走马观花’ 了? 没有强制去读 memset 相关的代码和具体实现, 思维量少了很多